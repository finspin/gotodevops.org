    ...
    <div class="conference row">
      ...
      <div class="col-3 col-md-3 weather">
        <div class="weather">
          ...
          />
          <font-awesome-icon
            v-show="conference.temperature.celsius >= 10 && conference.temperature.celsius < 20"
            icon="cloud"
            :style="{ color: '#3f5167' }"
            class="mr-1"
          />
          <font-awesome-icon
            v-show="conference.temperature.celsius >= 20"
            icon="umbrella-beach"
            :style="{ color: '#3f5167' }"
            class="mr-1"
          />
      ...
      <div class="col-4 col-md-3 pl-0">
        <div class="cost float-right">
          ...
          <div v-show="conference.cost === 1">&lt; $300 / DAY</div>
          <div v-show="conference.cost === 2">&lt; $600 / DAY</div>
          <div v-show="conference.cost === 3">> $600 / DAY</div>
...
export default {
  ...
  methods: {
    conferenceDateRange: conference => {
      ...
      const confEndMonth = confEndDate
        .toLocaleString(locale, { month: "short" })
        .toUpperCase();
      ...
      if (confStartDay === confEndDay && confStartMonth === confEndMonth) {
        confDateRange =
          confStartDay + " " + confStartMonth + " " + confStartYear;
      } else if (
        confStartMonth === confEndMonth &&
        confStartYear === confEndYear
      ) {
        confDateRange =
          confStartDay +
          " - " +
